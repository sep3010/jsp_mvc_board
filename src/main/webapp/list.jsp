...
<body class="container">
	...
	<form class="form-inline justify-content-end mt-5 mb-3" action="searchResult.do" method="get">
	   <label class="my-1 mr-2" for="textSearch">
	   	<i class="fas fa-search fa-2x" style="color: #D2BBF2;"></i>
	   </label>
	  ...
	  <select class="custom-select mx-2" id="dateSearch" name="dateSearch">
	    ...
	    <option value="all" selected>전체 날짜</option>
	  </select>
	  ...
	  <select class="custom-select mr-2" id="textSearch" name="textSearch">
	    ...
	    <option value="total" selected>전체 글</option>
	  </select>	 
	  
	  <input type="text" class="form-control mr-2" placeholder="검색어를 입력하세요." name="searchWord">
	...
	<table class="table table-borderless mb-1">
		...
		<tfoot>
			<tr>
			 	<td colspan="5">
                    <a href="write_view.do" class="btn px-4" style="color: black;">글작성   <i class="fas fa-pencil-alt fa-1x"></i></a>
                </td>
			</tr>
		</tfoot>
	...
	<div class="text-center" style="color: #8E6BBF; font-size: 22px;">
		<c:if test="${pageMaker.pre}">
	       <a href="list.do${pageMaker.makeQuery(pageMaker.startPage - 1) }" style="color: #8E6BBF;"><img src="img/fast-backward.png"></a>
	    </c:if>
	    
	    <c:if test="${currentPageNum > 1}">
	       <a href="list.do${pageMaker.makeQuery(currentPageNum - 1)}" class="mx-1"><img src="img/previous.png"></a>
	    </c:if>
	    ...
	    <c:forEach var="idx" begin="${pageMaker.startPage}" end="${pageMaker.endPage}" >
	    	<c:choose>
	    		...
	    		<c:otherwise>
	    			<a href="list.do${pageMaker.makeQuery(idx)}" style="color: #8E6BBF;" class="mx-1">${idx}</a>
	    		</c:otherwise>	    	
	    	</c:choose>    	
	    </c:forEach>
	    
	    <c:if test="${currentPageNum < pageMaker.realEnd}">
	       <a href="list.do${pageMaker.makeQuery(currentPageNum + 1) }" style="color: #8E6BBF;"><img src="img/next.png"></a>
	    </c:if>
	      
	    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
	       <a href="list.do${pageMaker.makeQuery(pageMaker.endPage +1) }" style="color: #8E6BBF;"><img src="img/fast-forward.png"></a>
	    </c:if>
    </div>
